...
def segmentation(n, seg_points, label, cls_labels, reg_targets, dataset_params, phase):
    ...
    top_prev = bcl_bn_relu(n, 'bcl_seg', top_prev, top_lattice, nout=[64, 128, 128, 64],
                          lattic_scale=["0*8_1*8_2*8", "0*4_1*4_2*4", "0*2_1*2_2*2", "0_1_2"], loop=4, skip=None)
    ...
    n.p2fm = L.Python(seg_points, top_prev,
                     name = "Point2FeatMapV4",
                     python_param=dict(
                     module='bcl_layers',
                     layer='Point2FeatMapV4' #V2 is for no seg score, V3 return (B,C,N,H,W), V4 batch version
                     ),
