...
def Eval_Solver(exp_dir, eval_weights, config_path, eval_proto_path, pretrained):
    solver = solver_function.SegEval(
                                eval_proto_path,
                                eval_weights,
...
def Seg_Eval_Solver(exp_dir, eval_weights, config_path, eval_proto_path, pretrained):
    solver = solver_function.SegEval(
                                eval_proto_path,
                                eval_weights,
                                os.path.join(exp_dir, 'pp'),
...
def caffe_model(args, restore, pretrained, start_eval, visual_log):
    ...
    if start_eval:
        try:
            recent_model = sut.restore_latest_checkpoints(exp_dir)
            evl_weights = os.path.join(exp_dir, "{}.caffemodel".format(str(recent_model)))
            # solver = Eval_Solver(exp_dir, evl_weights, config_path, eval_proto_path, pretrained)
            solver = Seg_Eval_Solver(exp_dir, evl_weights, config_path, eval_proto_path, pretrained)
...
def train(  config_path, model_dir,
            restore=False, start_eval=False,
            pretrain = False,
            pretrain_path='./exp/LEO_BaseLine',
            pretrain_weight_name='pp_iter_66816.caffemodel',
            visual_log=False):

    pretrained = None
    if pretrain:
        pretrained = {}
        pretrained['pretrain_path'] = pretrain_path
        pretrained['pretrain_weight_name'] = pretrain_weight_name
