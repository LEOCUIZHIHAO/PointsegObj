...
class AnchorGeneratorStride(AnchorGenerator):
    ...
    def generate_from_voxels(self, coords):
        return box_np_ops.voxels_anchors_3d_generator(
            coords, self._sizes, self._anchor_strides,
            self._anchor_offsets, self._rotations, self._dtype)
...
class AnchorGeneratorRange(AnchorGenerator):
    def __init__(self,
        ...
        self.unmatch_threshold = unmatch_threshold
        self._custom_values = custom_values
        # # TODO: read from prototxt
        self._anchor_strides = [0.05, 0.05, 0]# 2d anchors //# [0.05, 0.05, 0.1] 3d anchors
        self._anchor_offsets = [0.025, -39.975, -1]# 2d anchor //s # 3d anchor ==>[0.025, -39.975, -2.95] #xyz(min) + voxel_size # TODO: # DEBUG:
        self._anchor_shift=[0.8, 1.95] #(horizontal=0.8 bouding box) , (vertical=1.95 bouding box)
    ...
    def generate_from_voxels(self, coords):
        return box_np_ops.voxels_anchors_3d_generator(
            coords, self._sizes, self._anchor_shift, self._anchor_strides,
            self._anchor_offsets, self._rotations, self._dtype)
