...
class TargetAssigner:
    ...
    def assign_per_class(self,
        ...
        for class_name, anchor_dict in anchors_dict.items():
            ...
            """ here make the number of proposals anchors\
                anchor_dict["anchors"] is the key to control the number of anchors
            """
        ...
        targets_dict['labels'] = targets_dict['labels'][0]
        targets_dict['bbox_targets'] = targets_dict['bbox_targets'][0]
        targets_dict['importance'] = targets_dict['importance'][0]
        ...
        #the label out is feature map size so need to be done something
        """
    ...
    def generate_anchors(self, feature_map_size):
        ...
        for anchor_generator, match_thresh, unmatch_thresh, fsize in zip(
            ...
            anchors = anchor_generator.generate(fsize)  #generate from feature map
            # jim added
    ...
    def generate_anchors_from_voxels(self, coordinates):
        anchors_list = []
        ...
        for anchor_generator, match_thresh, unmatch_thresh in zip(
            ...
            anchors = anchor_generator.generate_from_voxels(coordinates) #generate from voxels
    ...
    def generate_anchors_dict_from_voxels(self, coordinates):
        ndim = len(coordinates)
        anchors_list = []
        matched_thresholds = [
            a.match_threshold for a in self._anchor_generators
        ...
        for anchor_generator, match_thresh, unmatch_thresh in zip(
                self._anchor_generators, matched_thresholds,
                unmatched_thresholds):
            anchors = anchor_generator.generate_from_voxels(coordinates)  ## NOTE: generate from voxels
            num_anchors = np.prod(anchors.shape[:-1])
            match_list.append(
                np.full([num_anchors], match_thresh, anchors.dtype))
            unmatch_list.append(
    ...
    def generate_anchors_from_gt(self, coordinates):
        anchors_list = []
        ndim = len(coordinates)
        matched_thresholds = [
            a.match_threshold for a in self._anchor_generators
        ...
        for anchor_generator, match_thresh, unmatch_thresh in zip(
            ...
            anchors = anchor_generator.generate_from_groundtruth(coordinates) #generate from gt
            anchors_list.append(anchors)
            num_anchors = np.prod(anchors.shape[:-1])
            match_list.append(
                np.full([num_anchors], match_thresh, anchors.dtype))
    ...
    def generate_anchors_dict_from_gt(self, coordinates):
        ndim = len(coordinates)
        anchors_list = []
        matched_thresholds = [
            a.match_threshold for a in self._anchor_generators
        ...
        for anchor_generator, match_thresh, unmatch_thresh in zip(
                self._anchor_generators, matched_thresholds,
                unmatched_thresholds):
            anchors = anchor_generator.generate_from_groundtruth(coordinates)  ## NOTE: generate from gt
            num_anchors = np.prod(anchors.shape[:-1])
            match_list.append(
                np.full([num_anchors], match_thresh, anchors.dtype))
            unmatch_list.append(
